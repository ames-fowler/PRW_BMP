...
component <- read.csv('EXTRACTIONS/PRW/SSURGO/PRW_SSURGO_component.csv')[,c('mukey','cokey','comppct.r','compname')]

#load corestrictions data

restrictions <- read.csv('EXTRACTIONS/PRW/SSURGO/PRW_SSURGO_corestrictions.csv')[,c('cokey','resdept.r')]
...
test <- ifelse(comp_r$resdept.r < 100, "medium", "deep")
test <- ifelse(comp_r$ksat_min/0.277778 < 10.0 & comp_r$hz_ksat_min_depth < 50, "shallow", "deeP")

color

hist(comp_r$ksat_min/0.277778, breaks=100, col=soil_depth)
...
?mutate
# For	the	HCW,	the	“shallow”	soils	in	the	eastern	region	were	therefore	defined	
...
writeRaster(Mapunit_r, "sratchspace/Mapunit_r")

Mapunit_r_C<- mask(Mapunit_r, NED_PRW)

writeRaster(Mapunit_r, "processed_data/Mapunit_r")
?sf()
