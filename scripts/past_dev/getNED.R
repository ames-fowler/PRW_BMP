...
# PRW extent  -------------------------------------------------------------
#use the larger extent to pass through FedData, then use shape file to clip to reproject and clip to size
...
# get NED -----------------------------------------------------------------
#get NED DEM at 1 acr ~30m for the PRW extent, mosaic the tiles, repoject and crop
...
#NED<-raster("EXTRACTIONS/PRW_NED/NED/PRW_NED_NED_1.tif") # old  
...
plot(NED_PRW)
writeRaster(NED_PRW,"processed_data/NED_PRW.tif", overwrite=F)
#NED_PRW <- raster("processed_data/NED_PRW.grd")
#plot(PRW_NED_clip, main="PRW NED DEM 30m", xlab="Easting", ylab="Northing")                              
                           
#plot(NED_PRW)
NED_PRW <- raster("processed_data/NED_PRW.tif") ########################### Check extent shifting 2.8 m north and 6 meters east from previous run 

#NED_test_PRW <- terrain(NED_PRW, opt=c('slope', 'aspect','flowdir'), unit='degrees')
#plot(x)
#NED_aspect_PRW <- terrain(NED_PRW,otp= "aspect", neighbors = 8)
#hist(NED_aspect_PRW)
#plot(NED_aspect_PRW)
#?terrain
#plot(NED_test_PRW$aspect)
